...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    if 'Daily' not in ret_freq_use:
        raise ValueError("`ret_freq_use` must include 'Daily' for volatility calculation.")

        # 从输入字典中提取日回报DataFrame
    daily_returns = ret['Daily'].copy()

    # 计算每个股票每月的波动性（标准差）
    daily_returns['Year_Month'] = daily_returns.index.to_period('M')
    vol_df = daily_returns.groupby('Year_Month').std()

    # 为列名添加后缀以表示波动性
    vol_df = vol_df.rename(columns=lambda x: x + f'_{cha_name}')
