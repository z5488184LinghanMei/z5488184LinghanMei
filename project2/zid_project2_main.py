...
def portfolio_main(tickers, start, end, cha_name, ret_freq_use, q):
    ...
    # Assuming dict_ret contains DataFrames for daily and monthly returns
    # Convert and save the daily returns DataFrame from dict_ret
    daily_returns_df = pd.DataFrame.from_dict(dict_ret['Daily'])
    monthly_returns_df = pd.DataFrame.from_dict(dict_ret['Monthly'])

    # Save Daily and Monthly returns into one CSV for simplicity here
    # You may choose to separate them based on your requirements
    with open('DM_Ret_dict.csv', 'w') as f:
        f.write('Daily Returns\n')
        daily_returns_df.to_csv(f)
        f.write('\nMonthly Returns\n')
        monthly_returns_df.to_csv(f)

    # Save the DataFrame containing monthly total volatility for each stock
    df_cha.to_csv('Vol_Ret_mrg_df.csv')

    # Save the DataFrame containing the constructed equal-weighted quantile and long-short portfolios
...
def get_avg(df: pd.DataFrame, year):
    ...
    """
    # Ensure index is in Period format for easy filtering
    ...
    # Calculate and return the mean, ignoring NaN values
    return yearly_data.mean()
...
def get_cumulative_ret(df):
    ...
    """
    # Calculate cumulative product along the columns, then subtract 1 to get returns
    cum_ret = (1 + df).prod() - 1
    return cum_ret
...
def t_stat(df: pd.DataFrame, column_name: str):
    """Calculates mean, t-stat, and number of observations for a specified column in DataFrame.
    ...
    -------
    pd.DataFrame
        DataFrame containing mean, t-statistic, and number of observations.
    """
    if column_name not in df.columns:
        raise ValueError(f"Column {column_name} not found in DataFrame.")

    column_data = df[column_name].dropna()
    n_obs = len(column_data)

    # Ensure there's enough data
    if n_obs < 2:
        raise ValueError("Not enough data to compute t-statistic.")

    ls_bar = column_data.mean()
    std_dev = column_data.std(ddof=1)  # ddof=1 provides the sample standard deviation
...
if __name__ == "__main__":
    pass
    # 这里假设您已经成功运行了portfolio_main函数
    tickers = cfg.TICKERS  # 从config.py导入的TICKERS列表
    start = '2000-12-29'
    end = '2021-08-31'
    cha_name = 'vol'
    ret_freq_use = ['Daily']
    ...
    # Q1: 2008年平均日回报最低的股票
    daily_returns_2008 = dict_ret['Daily'][dict_ret['Daily'].index.year == 2008].mean()
    min_daily_return_2008_stock = daily_returns_2008.idxmin()
    Q1_ANSWER = min_daily_return_2008_stock

    # Q2: 该股票2008年的平均日回报
    Q2_ANSWER = daily_returns_2008[min_daily_return_2008_stock]

    # Q3: 2019年平均月回报最高的股票
    monthly_returns_2019 = dict_ret['Monthly'][dict_ret['Monthly'].index.year == 2019].mean()
    max_monthly_return_2019_stock = monthly_returns_2019.idxmax()
    Q3_ANSWER = max_monthly_return_2019_stock

    # Q4: 该股票2019年的平均月回报
    Q4_ANSWER = monthly_returns_2019[max_monthly_return_2019_stock]

    # Q5: TSLA 2010年的平均月度总波动率
    tsla_vol_2010 = df_cha[df_cha.index.year == 2010]['tsla_vol'].mean()
    Q5_ANSWER = tsla_vol_2010

    # Q6: Visa股票2008年和2018年平均月度总波动率的比率
    v_vol_2008 = df_cha[df_cha.index.year == 2008]['v_vol'].mean()
    v_vol_2018 = df_cha[df_cha.index.year == 2018]['v_vol'].mean()
    Q6_ANSWER = v_vol_2018 / v_vol_2008

    # Q7: TSLA在2010年的有效年月数量
    tsla_effective_months_2010 = df_cha[df_cha.index.year == 2010]['tsla_vol'].dropna().count()
    Q7_ANSWER = tsla_effective_months_2010

    # Q8: EW_LS_pf_df数据框的行和列数量
    rows, cols = df_portfolios.shape
    Q8_ANSWER = f"{rows}, {cols}"

    # Q9: 2019年波动性最低的分位数的平均等权重投资组合回报
    lowest_vol_portfolio_return_2019 = df_portfolios[df_portfolios.index.year == 2019]['ewp_rank_1'].mean()
    Q9_ANSWER = lowest_vol_portfolio_return_2019

    # Q10: 整个样本期间波动性长短投资组合的累积回报
    cumulative_ls_return = (df_portfolios['ls'] + 1).prod() - 1
    Q10_ANSWER = cumulative_ls_return
    ...
    print("Q9 Answer:", Q9_ANSWER)
    print("Q10 Answer:", Q10_ANSWER)
    dict_ret, df_cha, df_portfolios = portfolio_main(tickers, start, end, cha_name, ret_freq_use, q)

    # Assuming t_stat function is defined earlier in the script
    result_df = t_stat(df_portfolios, 'ls')
    print("T-Stat Analysis of 'ls' column in EW_LS_pf_df:")
    print(result_df)
